...
export default function LoginCheckContent() {
  ...
  const [ageAgreed, setAgeAgreed] = useState<boolean>(false);
  ...
  const handleIndividualChange = (
    ...
    setTimeout(() => {
      ...
      const newPrivacyAgreed =
        setter === setPrivacyAgreed ? value : privacyAgreed;
      ...
      <div className="w-full max-w-[335px] flex flex-col gap-8">
        ...
        <div className="flex flex-col">
          ...
          <Checkbox
            checked={ageAgreed}
            onChange={(checked) =>
              handleIndividualChange(setAgeAgreed, checked)
            }
            label="만 14세 이상입니다"
            required
          ...
          <Checkbox
            checked={termsAgreed}
            onChange={(checked) =>
              handleIndividualChange(setTermsAgreed, checked)
            }
            label="서비스 이용 약관 동의"
            required
            href="/policy/terms"
          ...
          <Checkbox
            ...
            onChange={(checked) =>
              handleIndividualChange(setPrivacyAgreed, checked)
            }
            label="개인정보 처리방침 동의"
            required
            href="/policy/privacy"
